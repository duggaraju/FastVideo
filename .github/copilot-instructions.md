...
Infrastructure starts at `infra/main.bicep`, with transport-specific modules under `infra/modules`. Kubernetes manifests now live under `deploy/k8s/base`, `deploy/k8s/overlays/*`, `deploy/k8s/components/providers/*`, and `deploy/k8s/jobs/*`; `scripts/deploy.ps1` generates a small runtime overlay, pre-renders the provider/runtime/scheduling Job templates, writes `deploy/rendered.yaml`, and applies that rendered manifest.
...
- The manifest is the handoff between analyzer and media Jobs. Add workflow data there, in both contract implementations, when downstream workers need it; keep the mounted Job templates for static pod-spec wiring and use environment variables only for per-submission/runtime values.
...
- Treat `deploy/k8s/base`, `deploy/k8s/overlays`, `deploy/k8s/components`, `deploy/k8s/jobs`, `deploy/ladder-profiles.json`, and Bicep sources as editable inputs. Do not hand-edit generated `deploy/rendered.yaml`, `deploy/.generated/`, or `infra/main.json`.
