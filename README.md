...
- `deploy/k8s/base/` - namespace, service account, RBAC, the transport-agnostic analyzer/completion Deployments and PDB, plus the shared `video-config` and `video-ladder-profiles` ConfigMaps. The base `video-config` now carries only shared settings: storage account/container identifiers, mount paths, encoding defaults, namespace/service-account defaults, workload identity client ID, and the media image references used when rendering job templates.
